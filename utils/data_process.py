...
def get_corpus(name):
    ...
    def merge_word_entity(word_list, tag_list):
        """
    ...
    for row in zip(df['text'], df['label']):
        # 句子分词、命名实体识别
        lac_result = lac.run(row[0].replace('\n', '').replace(' ', ','))
        words.extend(lac_result[0])
        out += merge_word_entity(lac_result[0], lac_result[1]) + row[1] + '\n'
